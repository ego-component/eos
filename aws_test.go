...
func init() {
	awsCmp = newS3Cmp(os.Getenv("BUCKET"), "")
}

func newS3Cmp(bucket string, shards string) *Component {
	s3ForcePathStyle := cast.ToBool(os.Getenv("S3_FORCE_PATH_STYLE"))
	newConfs := fmt.Sprintf(s3Confs, os.Getenv("AK_ID"), os.Getenv("AK_SECRET"), os.Getenv("ENDPOINT"),
		bucket, s3ForcePathStyle, os.Getenv("REGION"), shards)
	fmt.Printf("newConfs--------------->"+"%+v\n", newConfs)
...
func TestS3_GetBucketName(t *testing.T) {
	...
	cmp := newS3Cmp(bucketShard, `"abcdefghijklmnopqr", "stuvwxyz0123456789"`)
...
func keys(meta map[string]string) []string {
	var res []string
	for k := range meta {
		res = append(res, k)
	}
...
func TestS3_Put(t *testing.T) {
	...
	meta["TEXT"] = "text"
	meta["X-SHIMO-UID"] = "uid"
	meta["x-Shimo-Eid"] = "eid"
	meta["X-Shimo-Tid"] = "tid"

	err = awsCmp.Put(ctx, S3Guid, strings.NewReader(S3Content), meta)
	assert.NoError(t, err)

	res, err := awsCmp.Head(ctx, S3Guid, keys(meta))
	fmt.Printf("res, err--------------->"+"%+v, %+v\n", res, err)
	assert.NoError(t, err)
}
